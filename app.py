...
def index():
    ...
    return render_template("index.html", database=transactions_db, cash=round(cash, 2), sum=round(sum, 2))
...
def register():
    ...
    if request.method == "GET":
        ...
        if not password:
            return apology("Must Give Confirmaton")
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
        user_cash = user_cash_db[0]["cash"]
        user_shares_db = db.execute("SELECT shares FROM transactions WHERE user_id = :id AND symbol = :symbol GROUP BY symbol", id=user_id, symbol=symbol)
